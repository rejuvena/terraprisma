...
echo "WARNING FOR ANYONE OFERRING SUPPORT: THIS IS A MODIFIED VERSION OF tModLoader (https://github.com/rejuvena/terraprisma)"
...
if [[ -f "$install_dir/dotnet" || -f "$install_dir/dotnet.exe" ]]; then
	...
	exec "$install_dir/dotnet" Rejuvena.Terraprisma.dll "$@" 2>"$NativeLog"
else
	echo "Launched Using System Dotnet"  2>&1 | tee -a "$LogFile"
	exec dotnet Rejuvena.Terraprisma.dll "$@" 2>"$NativeLog"
fi
